...
def generate_employee_no(position: str) -> str:
    """根据岗位生成下一个员工编号，格式：前缀-10001"""
    prefix = POSITION_PREFIX.get(position, "YG")
    max_seq = 10000  # 从10001开始
...
def create_employee(data: dict) -> dict:
    ...
    if not emp.employee_no:
        emp.employee_no = generate_employee_no(emp.position or "")
    if not emp.qr_code:
