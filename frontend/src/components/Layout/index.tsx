...
import {
  ...
  Warehouse, Users, FileText, UserCircle, MapPin, Menu, X, QrCode
} from 'lucide-react';
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* 桌面端侧栏 */}
      <aside className={`fixed top-0 left-0 h-full bg-wms-dark text-white z-40 transition-all
        ${isMobile ? (sidebarOpen ? 'w-64' : '-translate-x-full') : 'w-64'}`}>
        <div className="p-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            <Warehouse className="w-8 h-8 text-wms-accent" />
            <div>
              <h1 className="text-lg font-bold">仓储管理</h1>
              <p className="text-xs text-gray-400">WMS v2.1</p>
            </div>
          </div>
        </div>
          ...
          {navItems.map(item => (
            <NavLink
              ...
              className={({ isActive }) =>
                ...
                ${isActive ? 'bg-wms-primary text-white' : 'text-gray-300 hover:bg-white/10'}`
              }
            >
        ...
        {isMobile && (
          <header className="sticky top-0 z-20 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Warehouse className="w-5 h-5 text-wms-primary" />
              <span className="font-semibold">仓储管理</span>
            </div>
            <div className="w-10" />
          </header>
        )}
      ...
      {/* 移动端底部导航 */}
      {isMobile && (
        <nav className="fixed bottom-0 left-0 right-0 z-20 bg-white border-t border-gray-200 safe-area-bottom">
          <div className="flex overflow-x-auto scrollbar-hide py-1 px-2">
              ...
              return (
                <NavLink
                  ...
                  className={`flex-shrink-0 flex flex-col items-center py-1 px-2 text-xs transition-colors
                    ${active ? 'text-wms-primary' : 'text-gray-500'}`}
                >
            ...
            <button
              ...
              className="flex-shrink-0 flex flex-col items-center py-1 px-2 text-xs text-gray-500"
            >
