...
export default function Dashboard() {
  ...
  const cards = [
    { label: "商品总数", value: data.total_products, icon: Package, accent: "violet" },
    { label: "总库存", value: data.total_stock, icon: Warehouse, accent: "emerald" },
    { label: "今日入库", value: data.inbound_today, icon: ArrowDownToLine, accent: "blue" },
    { label: "今日出库", value: data.outbound_today, icon: ArrowUpFromLine, accent: "amber" },
    { label: "库存预警", value: data.alert_count, icon: AlertTriangle, accent: "rose" },
    { label: "发票", value: data.total_invoices, icon: DollarSign, accent: "slate" },
    { label: "待入库", value: data.pending_inbound, icon: ArrowDownToLine, accent: "sky" },
    { label: "待出库", value: data.pending_outbound, icon: ArrowUpFromLine, accent: "orange" },
  ];
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {cards.map((c) => (
          <div key={c.label} className={`bg-white rounded-xl border border-slate-200/80 border-l-4 p-4 shadow-sm ${accentStyles[c.accent]}`}>
